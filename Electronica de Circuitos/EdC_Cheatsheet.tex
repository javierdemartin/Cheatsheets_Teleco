	...
	\begin{center}
		\begin{tikzpicture}[scale = 0.8, transform shape]
			\draw (0,0) node[pnp](pnp){}
				(pnp.E) to[short,-o] (0,1)
    				(pnp.C) to[short,-o] (0,-1)
				(pnp.B) to[short,-o] (-1,0)
				
				(-1,0) node[anchor=east] {B} 
				(0,1) node[anchor=west] {E} 
				(0,-1) node[anchor=west] {C};
			\draw[-latex] (.4,.9) -- (.4,.2); 
			\draw (.4,.5) node[anchor=west] {$I_{C}$} ;
			\draw[-latex] (.4,-.2) -- (.4,-.9); 
			\draw (.4,-.5) node[anchor=west] {$I_{E}$} ;
			\draw[-latex] (-.2,1) -- (-1,0.2); 
			\draw (-.5,.8) node[anchor=east] {$-V_{BE}$};
			
			\begin{scope}[shift={(4,0)}]
			\draw (0,0) node[npn](npn){}
				(npn.E) to[short,-o] (0,-1)
    				(npn.C) to[short,-o] (0,1)
				(npn.B) to[short,-o] (-1,0)
...
\textbf{Región Activa}:  $I_C$ depende directamente de la corriente de base $I_B$, de la ganancia de corriente $\beta$ y de las resistencias conectadas al colector y emisor. En esta región se produce amplificación de la señal.\\

\textbf{Región Inversa}: \\

\textbf{Región de Saturación}: $I_C = I_E = I_{max}$. La corriente depende del $V_{CC}$ y de las resistencias conectadas al colector y emisor. Este modo aparece cuando la corriente de base es lo suficientemente grande como para inducir una corriente de colector $\beta$ veces más grande.\\

\textbf{Región de Corte}:  $I_C = I_E = 0$. $V_{CE}$ es la alimentación del circuito, al no haber corriente circulando no hay caí­da tensión. Este modo se da cuando $I_B = 0$.\\
...
\begin{circuitikz}[scale=.8,american voltages, american currents, transform shape]
	...
	\draw (npn.C) node[anchor=north] {C} % Collector
    	(npn.B) node[anchor=west] {B} % Gate
        (npn.E) node[anchor= north] {E} % Emisor
        ;
...
%        (-1, 0) [short, -*] to (-1, 1)
	...
	\draw (npn.C) node[anchor=east] {C} % Collector
    	(npn.B) node[anchor=south] {B} % Gate
        (npn.E) node[anchor= east] {E} % Emisor
        ;
...
%        (-1, 0) [short, -*] to (-1, 1)
	...
	\draw (npn.C) node[anchor=east] {C} % Collector
		...
		\begin{scope}[scale = .7, shift = {(1,2.5)}]
			\draw (0,0) to [short,i_=$i_b$, o-] (1,0)
				to [short, -] (1,-1)
...
Entrada por base y salida por emisor.\\

Alta impedancia de entrada, muy baja impedancia de salida, alta ganancia de corriente y unidad, o menor, ganancia de tensión.

\section{\underline{Transistor FET}}

\begin{equation*}
	g_{m} = \frac{2}{|V_{T}|} \sqrt{I_{D} \cdot I_{DSS}}
\end{equation*}
	...
	\begin{center}
		\begin{tikzpicture}[scale = 0.6, transform shape]
			...
			\draw (0,-1.55) node[above] {nMOS Dep};
			\end{scope}
			
			\begin{scope}[shift = {(6,0)}]
			\draw (0,0) node[pigfetd](pigfetd){}
			...
			\draw (0,-1.55) node[above] {nMOS Ac};
			\end{scope}
			...
			\draw (0,-1.55) node[above] {pMOS Ac};
			\end{scope}
		\end{tikzpicture}
...
\section{\underline{Etapas de Potencia}}

\subsection{Clase A}

\subsection{Clase B}

\section{\underline{Circuitos de Protección de Cortocircuitos}}



\section{\underline{Amplificador Operacional}}
...
Impedancia de salida:
	...
	\begin{enumerate}
	\item Eliminar la excitación de entrtada
	\item Sustituir $R_{L}$ por un generador de tensión.
	\end{enumerate}
...
\subsubsection{Potencia Disipada en una Carga}

\begin{equation*}
	P_{R_{L}} = \frac{2 \cdot V_{CC}^{2}}{\pi^{2} \cdot R_{L}}
\end{equation*}

\section{\underline{Ejercicios con Opamps}}
	...
	\item \textbf{Identificar el tipo de alimentación}:
		...
		\end{enumerate}
			...
			\begin{IEEEeqnarray*}{rCl}
				\text{Entrada (Realimentación)} \begin{cases}
				\text{Serie} \rightarrow \text{Tensión}\\
				\text{Paralelo} \rightarrow \text{Corriente}
 				\end{cases}\\
				\text{Salida  (Muestreo)} \begin{cases}
