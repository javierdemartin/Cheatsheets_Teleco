...
\begin{tabular}{|c|c|c|}
\hline
Base ComÃºn                              & Emisor ComÃºn & Colector ComÃºn \\ \hline
	...
	\begin{center}
		\begin{circuitikz} %[scale=1]
			\draw (0,0) node[npn](npn1) {$Q_1$}
 			...
 			\draw (-1.15,-4) node {$h_{fe_{CP}} = h_{f_{ek}}$};
 			\draw (0.15,-4.5) node {$h_{oe_{CP}} = h_{oe_k} \cdot n = \frac{I_{C_k} \cdot n}{V_A} = \frac{I_{C_P}}{V_A}$};
 			
		\end{circuitikz}
	\end{center}

Se comporta como \textbf{un Ãºnico transistor}, necesita una resistencia $R$ (\textit{ballast resistor}) para estabilizar el reparto de corriente entre los transistores. Permite trabajar en \textbf{altas corrientes}.\\
...
\textbf{Amplificador de continua}, simÃ©trico con 2 entradas y 2 salidas. Amplifica exclusivamente la diferencia de sus entradas: $V_O = k \cdot (V_{i1} - V_{i2})$. Tiene los siguientes modos de funcionamiento:

	\begin{itemize}
		\item \textbf{Salida Diferencial}: La señal de salida se toma entre las salidas de cada semi-red ($V_O = V_{O_1} - V_{O_2}$).
		\item \textbf{Salida AsimÃ©trica}: La señal de salida se toma de la salida de una semi-red ($V_O = V_{O_x}$). Para este modo es necesaria una $R_{EE}$ elevada.
	\end{itemize}

Tiene efectos parÃ¡sitos como las \textbf{corrientes de polarización} que provocan caí­das de tensión en las resistencias internas de las fuentes de señal a amplificar.
...
\begin{center}
		\begin{circuitikz}[scale=.5,american voltages, american currents, transform shape]
			...
			\draw[red, thick] (12,.95) sin (12.25,1.95) cos (12.5,.95) sin (12.75,.05) cos (13,.95);
		\end{circuitikz}
	\end{center}
...
\textbf{Clase AB}: El transistor conduce durante algo \textbf{mÃ¡s de medio ciclo}.\\
...
\subsubsection{Consideraciones TÃ©rmicas}
