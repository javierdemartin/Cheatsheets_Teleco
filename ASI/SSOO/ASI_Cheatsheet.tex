...
Los sistemas operativos tienen dos funciones principales: 
	...
	\begin{itemize}
		\item Proveer abstracciones a los programas de usuarios.
		\item Gestionar los recursos de los programas.
	\end{itemize}
...
¿Cómo evitar condiciones de carrera? Hay que prohibir que más de un proceso lean o escriban datos compartidos a la vez, llamado \textbf{exclusión mutua}.\\
...
?`Cómo resolver el \textbf{problema productor-consumidor} en el paso de mensajes sin memoria compartida? Suponiendo que todos los mensajes son del mismo tamaño y que todos los mensajes enviados pero no recibidos son guardados en el buffer automáticamente por el S.O.  El consumidor empieza enviando \texttt{N} mensajes vacíos el productor. Siempre que el productor tenga un item que dar al consumidor, coge un mensaje vacío y devuelve uno lleno. De esta forma, el número total de mensajes en el sistema se mantiene constante en el tiempo, por lo tanto pueden ser guardados en un espacio de memoria conocido. Si el productor  trabaja más rápido que el consumidor, todos los mensajes acabarán llenos, esperando al consumidor; el productor será bloqueado, esperando a una respuesta vacía. Si el consumidor trabaja más rápido, ocurre lo contrario: todos los mensajes serán vacío esperando al productor que los llene; el consumidor será bloqueado, esperando a un mensaje lleno.
	...
	\begin{enumerate}
	\item Pipes: Todos los mensajes del mismo tamaño, el S.O. almacena mensajes enviados y no recibidos.
...
\begin{figure}[h]
  ...
  \end{figure}
	...
	\centering
		...
		fill=white,
		rectangle, 
		minimum width={width("XXXXXX")+2pt},
		minimum height = 20pt,
		font=\small}
		]

		\node[draw](f){FORK EXEC};
		\node[draw, right=.4cm of f, text width = 1.7cm] (tabla){Tabla de Procesos};
		\node[draw, right=.4cm of tabla, text width = 1.6cm] (exit){EXIT SIGNAL};
...
font=\small}]
		...
		\node[block, right = 1.5cm of user](user2){Procesos de Usuario};
		\node[block,below=0cm of user2](device2){GM | SA};
		\node[block,below=0cm of device2](drivers2){I/O Tasks};
		\node[block,below=0cm of drivers2](interr2){Gestión de Procesos};
		
		\draw[thick,-stealth] (drivers2.10) -| (9,-1.5) node[pos= 0.4, above right]{Kernel};    
		\draw[thick,-stealth] (drivers2.190) -| (3,.5) node[pos= 0.5, below]{Usuario};    
...
La planificación del disco:
	...
	\begin{itemize}
	\item \textbf{Multiprogramación}: Procesos generan solicitudes de acceso a disco (leer, escribir) más deprisa de lo que pueden ser atendidas (movimiento de la cabeza de disco).
	\item \textbf{Planificación}: Examen detallado de las solicitudes pendientes para determinar el modo más eficiente de atenderlas.
	\end{itemize}
...
Los \textbf{relojes} son esenciales para la operación de cualquier sistema multiprogramado. Mantienen el tiempo del día y preveen a un proceso monopolizar el uso de la CPU
 
 \subsubsection{Hardware del Reloj}
 
 Dispositivo que genera interrupciones en intervalos definidos de tiempo.
 
 \subsubsection{Software del Reloj}
...
Asocia a cada archivo una estructura de datos llamada \textbf{i-nodo} (\textit{i-node}), que almacena los atributos y direcciones de disco de los bloques del archivo. Dado un i-nodo, es posible, encontrar todos los bloques de un archivo.

	\quad La principal ventaja de este esquema sobre las listas enlazadas es que los i-nodos sólo tienen que estar en memoria cuando el correspondiente archivo está abierto. 
	
	\quad Si cada i-nodo ocupa $n$ bytes y hay un número máximo de $k$ archivos que pueden ser abiertos a la vez, la memoria total ocupada por el array que alberga los i–nodos para los archivos abiertos ocupa $k \cdot n$ bytes. Y sólo este tamaño tendrá que ser reservado por adelantado.\\
...
fill=white,
rectangle, 
minimum width={width("Dirección de disco del bloque 4")+2pt},
minimum height = 20pt,
font=\small}]
		\node[block,below=0cm of acc](gyro){Atributos de Archivo};
		
		\node[block,below=0cm of gyro](mag){Dirección de disco del bloque 0};
		\draw[thick,-stealth] (mag.0) -- (4,-1.3);
		
		\node[block,below=0cm of mag](Datos){Dirección de disco del bloque 1};
		\draw[thick,-stealth] (Datos.0) -- (4,-2.0);
		
		\node[block,below=0cm of Datos](Codigo){Dirección de disco del bloque 2};
		\draw[thick,-stealth] (Codigo.0) -- (4,-2.7);
		
		\node[block,below=0cm of Codigo](b3){Dirección de disco del bloque 3};
		\draw[thick,-stealth] (b3.0) -- (4,-3.45);
		\node[block,below=0cm of b3](b4){Dirección de disco del bloque 4};
		\draw[thick,-stealth] (b4.0) -- (4,-4.2);
		\node[block,below=0cm of b4](b5){Dirección de disco del bloque 5};
		\draw[thick,-stealth] (b5.0) -- (4,-4.9);
		\node[block,below=0cm of b5](b6){Dirección de disco del bloque 6};
		\draw[thick,-stealth] (b6.0) -- (4,-5.6);
		\node[block,below=0cm of b6](b7){Dirección de disco del bloque 7};
		\draw[thick,-stealth] (b7.0) -- (4.7,-6.32);
		
		\node[block, right = 2.5cm of b7, text width = 3.9cm](disco){Bloque de disco que contiene direcciones de disco adicional};
		
	\end{tikzpicture}	
	}
      \caption{Ejemplo de un i-nodo}
  \end{figure}
...
Distribución típica del Sistema de Archivos:
	...
	\begin{enumerate}
		\item \textbf{Bloque Boot}: Ocupa 1 bloque.
		\item \textbf{Super Bloque}: Información de la distribución del Sistema de Archivos (su tarea principal es dar información del tamaño de las 6 partes que lo forman). Ocupa 1 bloque.
		\item \textbf{Mapa de Bits de los i-nodos}: Es necesario un 1 bit para referenciar cada archivo.
		\item \textbf{Mapa de bits de las zonas}: 
%		\resizebox {\columnwidth} {!} {
		\begin{equation*}
		\resizebox {.92\columnwidth} {!} {%
			$\frac{\text{Tamaño total}}{\text{Tamaño de bloque}} = \text{zonas}\rightarrow  \frac{\text{zonas}}{\text{8}} = \text{bytes} \rightarrow  \frac{\text{Bytes}}{\text{Tamaño Bloque}} = \text{bloques}$%
			}
		\end{equation*}
%		}
		\item \textbf{i-nodos}:
		...
		\item Datos: Bloques libres que queden.
	\end{enumerate}	
...
En MINIX, el seguimiento de los i-nodos y de las zonas libres se realiza a través de dos mapas de bits. 
	...
	\quad Cuando se crea un archivo, el Sistema de Archivos busca a través de los bloques de mapas de bits hasta que encuentra un i-nodo libre. Si todos los i-nodos de unbloque están ocupados, la rutina de búsqueda devuelve un 0, por eso nunca se emplea el i-nodo 0.
...
La implementación de la memoria caché del SA está formada por buffers. Cada uno de ellos posee una cabecera y un bloque de disco. Todos los bloque están formando una lista doblemente encadenada o enlazada, teniendo al inicio de la lista el bloque menos recientemente usado (LRU) y en la última el más recientemente usado.
...
Como resultado, no es posible poner la posición en la tabla de procesos. La posición debe ser compartida.
	...
	\quad La solución es introducir una nueva tabla, \textbf{FILP} (\text{File Position}) que contiene las posiciones de los archivos.
	
\begin{figure}[!ht]
	\centering
	\includegraphics[width = .2\textwidth]{filp}
\end{figure}
